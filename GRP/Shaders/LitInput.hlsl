#ifndef GRP_LIT_INPUT_INCLUDED
#define GRP_LIT_INPUT_INCLUDED
TEXTURE2D(_BaseMap);
TEXTURE2D(_MaskMap);
...
float4 GetBase (InputConfig c) {
    ...
    if (c.useDetail)
    {
        ...
        map.rgb = lerp(sqrt(map.rgb),detail.r < 0.0 ? 0.0 : 1.0 , abs(detail.r) * mask);
        map.rgb *= map.rgb;
    }
...
float GetFersnel(InputConfig c)
{
